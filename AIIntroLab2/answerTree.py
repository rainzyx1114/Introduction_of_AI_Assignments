...
def entropy(Y: np.ndarray):
    ...
    """
    # TODO: YOUR CODE HERE
    _, Ycnt = np.unique(Y, return_counts=True)
    Yp = Ycnt / Y.shape[0]
    entro = - np.sum (Yp * np.log2(Yp))
    return entro
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # TODO: YOUR CODE HERE
    for _, v in enumerate(ufeat):
        root.children[v] = buildTree(X[feat == v, :], Y[feat == v], unused, depth - 1, purity_bound, gainfunc)
    return root
