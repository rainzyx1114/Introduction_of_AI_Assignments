...
class sigmoid(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        local_grad = self.cache[-1] * (1 - self.cache[-1])
        return np.multiply(grad, local_grad)        
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        # TODO: YOUR CODE HERE
        w, _ = self.params
        self.grad.append(np.matmul(self.cache[-1].T, grad))
...
class CrossEntropyLoss(Node):
    ...
    def cal(self, X):
        ...
        y = self.y
        self.cache.append(X)
        return -np.sum(np.log(np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1) + 1e-9))
    ...
    def backcal(self, grad):
        ...
        X, y = self.cache[-1], self.y
        ret = np.zeros_like(X)
        vals = -1 / (np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1) + 1e-9)
        np.put_along_axis(ret, np.expand_dims(y, axis=-1), vals, axis=-1)
        return grad * ret