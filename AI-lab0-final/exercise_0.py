...
def q5():
    ...
    '''
    n = 10000000
    points = np.random.uniform(-1.0, 1.0, (n, 2))
    distances = points[:, 0]**2 + points[:, 1]**2
    res = 4.0*np.sum(distances <= 1) / n
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        layout_grid = [[' ' for _ in range(self.width)] for _ in range(self.height)]
        ...
        if self.pacman_pos is not None:
            ...
            if 0 <= x < self.height and 0 <= y < self.width:
                layout_grid[x][y] = 'P'
        temp = [''.join(row) for row in layout_grid]
        layout_text = '\n'.join(temp)
        return layout_text
def q6(file_path):
    '''
