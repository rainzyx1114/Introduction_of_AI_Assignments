    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in state.getLegalActions(index_of_agent):
                s = state.generateSuccessor(index_of_agent, action)
                v = minimizer(s, depth, 1)[0]
                if v > value:
                    value = v
                    maxiAction = action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in state.getLegalActions(index_of_agent):
                s = state.generateSuccessor(index_of_agent, action)
                if index_of_agent != s.getNumAgents() - 1:
                    v = minimizer(s, depth, index_of_agent + 1)[0]
                else:
                    v = maximizer(s, depth - 1, 0)[0]
                if (v < value):
                    value = v
                    miniAction = action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                s = state.generateSuccessor(index_of_agent, action)
                v = minimizer(s, depth, 1, alpha, beta)[0]
                if v > value:
                    value = v
                    maxiAction = action
                    if value > beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                ...
                if index_of_agent != state.getNumAgents() - 1:
                    v = minimizer(s, depth, index_of_agent + 1, alpha, beta)[0]
                else:
                    v = maximizer(s, depth - 1, 0, alpha, beta)[0]
                if v < value:
                    value = v
                    miniAction = action
                    if value < alpha:
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            ...
            complete = True
            while True:
                if cgs.isWin() == True or cgs.isLose() == True:
                    break
                children = []
                for act in cgstree.statevalue.getLegalActions(0):
                    if act == 'North' and cgstree.north is not None:
                        children.append((cgstree.north, act))
                    elif act == 'South' and cgstree.south is not None:
                        children.append((cgstree.south, act))
                    elif act == 'West' and cgstree.west is not None:
                        children.append((cgstree.west, act))
                    elif act == 'East' and cgstree.east is not None:
                        children.append((cgstree.east, act))
                    elif act == 'Stop' and cgstree.stop is not None:
                        children.append((cgstree.stop, act))
                    else:
                        complete = False
                        break
                if not complete:
        ...
        def Expansion(cgstree):
            ...
            for action in legal_actions:
                data = [cgstree.statevalue.generateSuccessor(0, action), 0, 1]
                if action == 'North' and cgstree.north is None:
                    cgstree.north = Node(data)
                    cgstree.north.parent = cgstree
                elif action == 'South' and cgstree.south is None:
                    cgstree.south = Node(data)
                    cgstree.south.parent = cgstree
                elif action == 'West' and cgstree.west is None:
                    cgstree.west = Node(data)
                    cgstree.west.parent = cgstree
                elif action == 'East' and cgstree.east is None:
                    cgstree.east = Node(data)
                    cgstree.east.parent = cgstree
                elif action == 'Stop' and cgstree.stop is None:
